...
export const csvMappingConfigs = pgTable("csv_mapping_configs", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  description: text("description"),
...
export const csvFieldMappings = pgTable("csv_field_mappings", {
  id: serial("id").primaryKey(),
  configId: integer("config_id").references(() => csvMappingConfigs.id).notNull(),
  fieldName: text("field_name").notNull(), // e.g., 'sourceId', 'targetId'
  csvColumn: text("csv_column").notNull(),
  required: boolean("required").default(false).notNull(),
  validationType: text("validation_type").notNull(), // e.g., 'string', 'number', 'date'
  validationRules: jsonb("validation_rules"), // Optional validation rules in JSON format
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const csvAttributeMappings = pgTable("csv_attribute_mappings", {
  id: serial("id").primaryKey(),
  configId: integer("config_id").references(() => csvMappingConfigs.id).notNull(),
  attributeDefinitionId: integer("attribute_definition_id")
    ...
    .notNull(),
  csvColumn: text("csv_column").notNull(),
  transformationType: text("transformation_type").notNull(), // e.g., 'enum', 'date', 'number'
  transformationRules: jsonb("transformation_rules"), // Transformation rules in JSON format
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

// Add relations
export const csvMappingConfigsRelations = relations(csvMappingConfigs, ({ many }) => ({
  fieldMappings: many(csvFieldMappings),
  attributeMappings: many(csvAttributeMappings),
}));

export const csvFieldMappingsRelations = relations(csvFieldMappings, ({ one }) => ({
  config: one(csvMappingConfigs, {
    fields: [csvFieldMappings.configId],
    references: [csvMappingConfigs.id],
  }),
}));

export const csvAttributeMappingsRelations = relations(csvAttributeMappings, ({ one }) => ({
  config: one(csvMappingConfigs, {
    fields: [csvAttributeMappings.configId],
    references: [csvMappingConfigs.id],
  }),
  attributeDefinition: one(relationshipAttributeDefinitions, {
    fields: [csvAttributeMappings.attributeDefinitionId],
    references: [relationshipAttributeDefinitions.id],
  }),
}));

// Add validation schemas
export const insertCsvMappingConfigSchema = createInsertSchema(csvMappingConfigs);
export const insertCsvFieldMappingSchema = createInsertSchema(csvFieldMappings);
export const insertCsvAttributeMappingSchema = createInsertSchema(csvAttributeMappings);

// Add types
export type CsvMappingConfig = typeof csvMappingConfigs.$inferSelect;
export type InsertCsvMappingConfig = z.infer<typeof insertCsvMappingConfigSchema>;
export type CsvFieldMapping = typeof csvFieldMappings.$inferSelect;
export type InsertCsvFieldMapping = z.infer<typeof insertCsvFieldMappingSchema>;
export type CsvAttributeMapping = typeof csvAttributeMappings.$inferSelect;
export type InsertCsvAttributeMapping = z.infer<typeof insertCsvAttributeMappingSchema>;